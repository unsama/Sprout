...
router.post('/confitable', function (req, res, next) {
    connection.query("select * from leave_type limit 2", function (error, results, fields) {
        if (error) res.json({"status": "failed", "message": error.message});
        else{
            res.json({"status": "ok", "data": results});
        }
...
router.post('/confitablenext', function (req, res, next) {
    ...
    connection.query('select * from leave_type limit 2  OFFSET '+""+ req.body.counter +"" +'', function (error, results, fields) {
        if (error) res.json({"status": "failed", "message": error.message});
        else{
            res.json({"status": "ok", "data": results});
        }
