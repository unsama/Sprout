...
router.post('/selectgrid', function (req, res, next) {
    ...
    connection.query("SELECT contact.id,contact.name,contact.street1,contact.street2,contact.city,contact.country,contact.email, tags_contacts.tag_id FROM contact left JOIN tags_contacts ON contact.id = tags_contacts.contact_id  limit 20", function (error, results, fields) {
        if (error) res.json({"status": "failed", "message": error.message});
        else{
            res.json({"status": "ok", "data": results});
        }
...
router.post('/selectgridnext', function (req, res, next) {
    ...
    connection.query('SELECT contact.id,contact.name,contact.street1,contact.street2,contact.city,contact.country,contact.email, tags_contacts.tag_id FROM contact left JOIN tags_contacts ON contact.id = tags_contacts.contact_id limit 20 OFFSET '+""+ req.body.counter +"" +'', function (error, results, fields) {
        if (error) res.json({"status": "failed", "message": error.message});
        else{
            res.json({"status": "ok", "data": results});
        }
